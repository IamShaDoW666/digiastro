...
const product = ({ product }: { product: Product }) => {
  return (
    <section className="mx-auto overflow-clip flex  max-w-6xl flex-col px-4 py-10 sm:px-6 lg:px-8 lg:py-14 2xl:max-w-full">
      ...
      <h1 className="block text-2xl font-bold tracking-tighter text-white  sm:text-2xl md:text-4xl lg:text-5xl">

      {product.data.main.title}
      </h1>
      </motion.div>
      <div className="flex flex-col items-center justify-between mt-2 space-y-4 sm:flex-row sm:space-y-4">
        <motion.div style={{marginRight:'8px'}}initial={{y: 200, opacity: 0}} animate={{y: 0, opacity: 1}} transition={{duration: 1, delay: 1}}>
   
          <p className="mb-8 max-w-prose text-pretty font-light text-white   sm:text-xl">
